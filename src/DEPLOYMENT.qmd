# Comprehensive Deployment Documentation

This document provides a complete overview of the deployment architecture, processes, and environments for the Missbrauch Kath Info Wiki MediaWiki instance.

## Table of Contents

1.  [System Architecture & Pipeline](#system-architecture--pipeline)
2.  [Workflows](#workflows)
    -   [Development](#development-workflow)
    -   [Staging & Testing](#staging--testing)
    -   [Production Deployment](#production-deployment)
3.  [Infrastructure Components](#infrastructure-components)
4.  [Security and Access](#security-and-access)
5.  [Troubleshooting](#troubleshooting)
6.  [Command Reference](#command-reference)

## System Architecture & Pipeline {#system-architecture--pipeline}

The MediaWiki instance follows a containerized microservices architecture with a clear separation between development, staging, and production environments. The pipeline ensures code quality through staging verification before reaching production, while data can be synchronized back from production to staging for realistic testing.

```{mermaid}
graph TB
    subgraph "Workstations"
        Dev[👨‍💻 Developer]
    end

    subgraph "Version Control"
        GitHub[📂 GitHub Repository]
    end

    subgraph "Environments"
        subgraph "Development – Local"
            DevEnv[🛠️ Dev Container<br/>Live Reload]
        end

        subgraph "Staging – Server"
            StagingEnv[🧪 Staging Container]
            StagingDB[🗄️ Staging DB]
        end

        subgraph "Production – Server"
            ProdEnv[🚀 Production Container]
            ProdDB[🗄️ Production DB]
            Backup[💾 Backups]
        end
    end

    %% Development Flow
    Dev -->|Code & Test| DevEnv
    Dev -->|Commit & Push| GitHub

    %% Deployment Flow
    GitHub -->|Deploy| StagingEnv
    StagingEnv -->|Validate| ProdEnv

    %% Data Flow
    ProdEnv -->|Read / Write| ProdDB
    ProdDB -->|Backup| Backup
    ProdDB -.->|Sync Data| StagingDB
    StagingEnv -->|Read / Write| StagingDB

    %% Access
    User[👤 End Users] --> ProdEnv
    Dev -->|SSH / Maintenance| ProdEnv

    style DevEnv fill:#e3f2fd
    style StagingEnv fill:#fff3e0
    style ProdEnv fill:#e8f5e8
```

### Environment Summary

| Environment | Port | Purpose | Docker Compose | Database |
|---------------|---------------|---------------|---------------|---------------|
| **Development** | 8080 | Local dev & testing | `docker-compose.dev.yml` | `db_data` (vol) |
| **Staging** | 8081 | Pre-prod testing | `docker-compose.prod.yml` | `staging_db_data` |
| **Production** | 8080 | Live research | `docker-compose.prod.yml` | `db_data` |

## Workflows {#workflows}

### Development Workflow {#development-workflow}

1.  **Setup**: Clone the repo and configure `.env`.
2.  **Run**: Start the environment with live reload. See [Start Development Environment](#cmd-dev-start).
3.  **Iterate**: Make changes, test locally. Logs are available via [View Logs](#cmd-dev-logs).
4.  **Update**: If schema changes are needed, run [Update Schema](#cmd-maint-update).

### Staging & Testing {#staging--testing}

Staging mirrors production. Before deploying new features:

1.  **Sync Data**: Pull fresh data from production to staging using [Sync Prod to Staging](#cmd-ops-sync).
2.  **Deploy Code**: Update the staging container with the new image.
3.  **Test**: Verify functionality on port 8081.

### Production Deployment {#production-deployment}

1.  **Build**: Create the Docker image locally. See [Build Image](#cmd-ops-build).
2.  **Transfer**: Move the image to the production server.
3.  **Deploy**: Load the image and restart services. See [Deploy to Production](#cmd-ops-deploy).
4.  **Post-Deploy**: Run maintenance scripts. See [Post-Deployment Tasks](#cmd-ops-post).

## Infrastructure Components {#infrastructure-components}

The system is built on a custom Dockerfile (`src/Dockerfile`) extending MediaWiki 1.43.1.

-   **Web Services**: MediaWiki (Prod/Staging), phpMyAdmin, Caddy File Server.
-   **Database**: MariaDB 11.
-   **Storage**: Local volumes for DB data and images; Shared storage for cluster work.

See the [**Dockerfile**](Dockerfile) for details.

## Security and Access {#security-and-access}

-   **Network**: Access via Jump Server + SSH Key.
-   **Application**: MediaWiki auth, Admin access control.
-   **Data**: Encrypted connections, regular backups.

## Troubleshooting {#troubleshooting}

Common issues (Service Down, DB Error, Storage Full) can be diagnosed by checking logs and resources.

-   **Check Status**: [Service Status](#cmd-trouble-status)
-   **View Logs**: [View Detailed Logs](#cmd-trouble-logs)
-   **Restart**: [Restart Services](#cmd-trouble-restart)
-   **Database**: [Check Connectivity](#cmd-trouble-db)

## Command Reference {#command-reference}

### Development Commands {#cmd-dev}

**Start Environment** {#cmd-dev-start}

``` bash
docker-compose -f src/docker-compose.dev.yml up -d --build
```

**View Logs** {#cmd-dev-logs}

``` bash
docker-compose -f src/docker-compose.dev.yml logs -f
```

### Operations Commands (Prod/Staging) {#cmd-ops}

**Build Image (Local)** {#cmd-ops-build}

``` bash
docker build -f src/Dockerfile -t mediawiki:latest .
docker image save mediawiki -o mediawiki_image.tar
```

**Deploy to Production** {#cmd-ops-deploy}

``` bash
# Transfer (replace user/host)
rsync -av -e "ssh -J user@jump-server" ./mediawiki_image.tar user@prod-server:/path/

# Load & Restart
docker load < mediawiki_image.tar
docker-compose -f src/docker-compose.prod.yml up -d
```

**Sync Prod to Staging** {#cmd-ops-sync}

``` bash
./src/sync_prod_to_staging.sh
```

**Post-Deployment Tasks** {#cmd-ops-post}

``` bash
docker exec mediawiki php maintenance/update.php --quick
docker exec mediawiki php maintenance/runJobs.php --maxjobs=1000
```

### Database Management {#cmd-db}

**Backup & Restore** {#cmd-db-backup}

``` bash
# Backup
./src/backup_db.sh

# Restore
./src/restore_db.sh backups/prod_backup_YYYY-MM-DD.sql
```

**Maintenance** {#cmd-maint-update}

``` bash
# Update Schema
docker-compose -f src/docker-compose.dev.yml exec mediawiki php maintenance/update.php --quick

# Run Jobs
docker-compose -f src/docker-compose.dev.yml exec mediawiki php maintenance/runJobs.php --maxjobs=1000
```

### Troubleshooting Commands {#cmd-trouble}

**Service Status** {#cmd-trouble-status}

``` bash
docker-compose -f src/docker-compose.prod.yml ps
docker ps -a
```

**View Detailed Logs** {#cmd-trouble-logs}

``` bash
docker logs mediawiki --tail 100
docker logs db --tail 100
```

**Restart Services** {#cmd-trouble-restart}

``` bash
docker-compose -f src/docker-compose.prod.yml restart
```

**Check Connectivity** {#cmd-trouble-db}

``` bash
docker exec -it db mariadb -u wikiuser -p -e "SHOW DATABASES;"
```

**Cleanup** {#cmd-trouble-clean}

``` bash
docker system prune -f
docker volume prune -f
```

## Server Information

-   **Server**: `server.domain.ltdh`
-   **Jump Server**: `jump-server.domain.ltd`
-   **Web Access**: `http://server.domain.ltdh:8080`

## Template and Form Import Procedures

⚠️ **Note**: Import XML files into Semantic MediaWiki with Cargo and PageForms.

**Recreate Data**

``` bash
php maintenance/runJobs.php
php extensions/Cargo/maintenance/recreateData.php --table Faelle
php extensions/Cargo/maintenance/recreateData.php --table Betroffene
php extensions/Cargo/maintenance/recreateData.php --table Beschuldigte
php extensions/Cargo/maintenance/recreateData.php --table Kirchliche_Institutionen
php extensions/Cargo/maintenance/recreateData.php --table Strafverfolgungsbehoerden
php extensions/Cargo/maintenance/recreateData.php --table Fachgremien
```